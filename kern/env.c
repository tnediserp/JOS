...
env_alloc(struct Env **newenv_store, envid_t parent_id)
{
	...
	// at the first time, this e is exactly envs[0]
	if (!(e = env_free_list))
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (va = start; va < end; va += PGSIZE)
	{
		...
		if ((error = page_insert(e->env_pgdir, pp, va, PTE_P | PTE_W | PTE_U)) 
		< 0)
		{
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	// use this to switch to the user process's virtual address space.
	// now cr3 points to e->env_pgdir.
	// otherwise, have problems with memset.
	...
	// switch back to the kernel virtual address space
	// now cr3 points to kern_pgdir.
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	int error;
	if ((error = env_alloc(&newenv_store, 0)) < 0)
		panic("env_create: %e\n", error);
